...
import { ShareIcon, ClipboardIcon, CheckIcon, PhotoIcon } from "@heroicons/react/24/outline";
...
interface ShareButtonProps {
  ...
  url: string;
  text?: string;
  /** IPO slug — enables the "Create Share Card" option */
  slug?: string;
}

export default function ShareButton({ title, url, text, slug }: ShareButtonProps) {
  ...
  return (
    <>
      <div ref={menuRef} className="relative">
        ...
        {open && (
          <div
            className="absolute right-0 top-full mt-1.5 w-48 rounded-md shadow-lg border border-gray-200 dark:border-[#252A31] bg-white dark:bg-[#111418] z-50 py-1 overflow-hidden"
          >
            <button
              onClick={copyLink}
            ...
            {slug && (
              <>
                <div className="h-px bg-gray-100 dark:bg-[#252A31] mx-2 my-1" />
                <button
                  onClick={() => { setShowCard(true); setOpen(false); }}
                  className="flex w-full items-center gap-2.5 px-3.5 py-2 text-[12.5px] text-[#1C317A] dark:text-[#93B4FF] font-medium transition-colors hover:bg-[#eef2ff] dark:hover:bg-[#1C317A]/10"
                >
      ...
      {/* Share Card Modal — lazy loaded */}
      {showCard && slug && (
        <Suspense fallback={null}>
          <ShareCardModal
            slug={slug}
            ipoName={title.replace(" IPO", "")}
